...
export async function netzwerkGraphLaden(tage = 0, kantenLimit = 80): Promise<NetzwerkGraph> {
    ...
    const kanten = await sql`
        ...
        ORDER BY artikel_anzahl DESC, b.von_id, b.zu_id
        LIMIT ${kantenLimit}
    `;
...
export async function netzwerkZentralitaet(tage = 0, limit = 20) {
    const zeitraum = artikelZeitraumBedingung(tage);
...
export async function netzwerkStaerksteVerbindungen(tage = 0, limit = 25) {
    const zeitraum = artikelZeitraumBedingung(tage);
    ...
    return sql`
        ...
        ORDER BY artikel_anzahl DESC, p1.name, p2.name
        LIMIT ${limit}
    `;
}

export async function netzwerkHtml(tage: number): Promise<string> {
    const zentral = await netzwerkZentralitaet(tage, 15);
    const verbindungen = await netzwerkStaerksteVerbindungen(tage, 20);

    if (zentral.length === 0 && verbindungen.length === 0) {
        return `<p class="muted">Keine Personen-Verbindungen im gewählten Zeitraum (Co-Mentions in gemeinsamen Artikeln).</p>`;
    }
    ...
    const verbindungenHtml = verbindungen.length === 0 ? "" : `
        <h4>Stärkste Verbindungen</h4>
        <table class="dossier-table">
            <thead><tr><th>Person A</th><th>Person B</th><th>Beziehung</th><th>Gemeinsame Artikel</th></tr></thead>
            <tbody>${verbindungen.map((v) => `
                <tr>
    ...
    return `
        <p class="muted">Basierend auf Co-Mentions: Personen, die im selben Artikel erwähnt werden.</p>
        ${zentralHtml}
        ${verbindungenHtml}
    `;
}
